...
int main() {
	...
	if (k.get_kangaroo_population() < 1) {
		cout << "please enter a positive time value" << endl;
		cin >> kangroo_input_population;
	}
...
// this part is suposed to be where the dingo and fox eat functions are called but for now just going to put the rabbit and kangroo die functions
		...
		if (r.get_rabbit_population() && k.get_kangaroo_population()) {
			int j = rand() % 2 + 0; //gives j a random value of either 0 or 1
			if ((i % 5) == 0) {
				if (j == 0) {
					r.die();
					r.die();
...
//cout << "it is day: "<< (i - 1) << endl;
	cout << "The final Rabbit population is " << r.get_rabbit_population() << endl;
	cout << "The final Kangaroo population is " << k.get_kangaroo_population() << endl;
	cout << "The final Dingo population is " << d.get_dingo_population() << endl;
	cout << "The final fox population is " << f.get_fox_population() << endl;
